...
gridCoordinatesRange :: (Int, Int) --grid 
...
data SnakeGame = Game --game structure
  { snakeBody :: Snake
  , direction :: Direction
  , food      :: Food
  , nextFoods :: [Food]
...
initialState :: StdGen -> SnakeGame --initiate snake at top-left corner going right
initialState seed =
  Game
...
getFoods :: StdGen -> [Food] --get cherries' position 
...
getNextHead :: SnakeGame -> Location --follow snake head
...
moveSnake :: SnakeGame -> SnakeGame --snake motion
...
growSnake :: SnakeGame -> SnakeGame --snake size ++
...
checkFoodEaten :: SnakeGame -> SnakeGame --check if snake "ate" cherry
checkFoodEaten game
...
eatFood game = game {food = head foodList, nextFoods = tail foodList} --consume cherry
  where
...
resetGame :: SnakeGame -> SnakeGame --reset game by redrawing snalke
resetGame game =
  game {snakeBody = initialSnakeBody, direction = initialDirection}

checkCollisionWithOwnBody :: Snake -> Bool --check if snake "bit" itself
checkCollisionWithOwnBody (snakeHead:snakeTail) = snakeHead `elem` snakeTail

teleportThroughWalls :: Location -> Location --draw snake from one side of the window to the other on the according axis
...
nextFrame :: Float -> SnakeGame -> SnakeGame --generate new frame
nextFrame _ = checkGameOver . checkFoodEaten . moveSnake

keyPressed :: String -> SnakeGame -> SnakeGame --check what key was pressed as input
keyPressed key
